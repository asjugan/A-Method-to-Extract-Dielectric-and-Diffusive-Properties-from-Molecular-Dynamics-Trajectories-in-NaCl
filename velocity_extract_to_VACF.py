...
# Split by type and component
for atom_label, atom_code in zip(["Na", "Cl"], [1, 2]):
    for comp_index, comp_label in zip(range(3), ["x", "y", "z"]):
        comp_data = vel_array_xyz[atom_types == atom_code, :, comp_index]
        np.savetxt(f"velocities_{atom_label}_{comp_label}.txt", comp_data, fmt="%.6f")

# Time summary
elapsed_time_fs = (timestep_last - timestep_first) * dt_fs
print(f"Saved velocity components for Na and Cl into 6 files (x, y, z)")
...
v_Na_x = np.loadtxt("velocities_Na_x.txt")
v_Na_y = np.loadtxt("velocities_Na_y.txt")
v_Na_z = np.loadtxt("velocities_Na_z.txt")
v_Cl_x = np.loadtxt("velocities_Cl_x.txt")
v_Cl_y = np.loadtxt("velocities_Cl_y.txt")
v_Cl_z = np.loadtxt("velocities_Cl_z.txt")
vacf_Na = compute_acf(v_Na_x, v_Na_y, v_Na_z)
vacf_Cl = compute_acf(v_Cl_x, v_Cl_y, v_Cl_z)
# Shape: (n_timesteps,)
np.savetxt("vacf_Na300.txt", vacf_Na)
np.savetxt("vacf_Cl300.txt", vacf_Cl)